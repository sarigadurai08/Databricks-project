...
def discover_project_root(
    ...
    """
    Dynamically locate the Healthcare_pipeline project root.

    Search order (no hardcoded repo / username assumptions):
      1. Explicit HEALTHCARE_LAKEHOUSE_ROOT env
    ...
    if nb_path is not None:
        ...
        # Also try Repo-style path without the leading notebook folder noise
        try:
            # /Workspace/Users/<user>/.bundle/... or /Repos/<repo>/...
    ...
    for base_name in ("/Workspace/Users", "/Workspace/Repos", "/Workspace"):
        ...
        if not base.exists():
            continue
        try:
            # Shallow: base itself + one level of children + common nesting
            candidates.append(base)
            for child in list(base.iterdir())[:80]:
                if not child.is_dir():
                    continue
                candidates.append(child)
                # Two levels deep is enough for Users/<email>/repo
                try:
    ...
    raise FileNotFoundError(
        "Could not locate Healthcare_pipeline project root "
        f"(marker {PROJECT_MARKER[0]}/{PROJECT_MARKER[1]}). "
        "Set HEALTHCARE_LAKEHOUSE_ROOT or open the notebook from inside the repo."
    )
...
def clear_project_modules(prefixes: Iterable[str] = PACKAGE_PREFIXES) -> list[str]:
    ...
    Databricks interactive sessions keep stale bytecode after Git pulls /
    source edits. Clearing the cache forces the next import to load fresh code
    without requiring a full cluster / session restart.
    """
...
def reload_project_modules(prefixes: Iterable[str] = PACKAGE_PREFIXES) -> list[str]:
    ...
    Returns the list of modules that were cleared.
    """
    removed = clear_project_modules(prefixes)
    # Eagerly re-import package roots so subsequent `from config...` works
    for pkg in prefixes:
        try:
...
def resolve_notebook_path(relative: str, dbutils: Any = None) -> str:
    ...
    Accepts:
      - relative paths like ./Bronze/01_bronze_ingestion
    ...
    if base is None:
        # Fall back to relative form (works when driver notebook lives in notebooks/)
        return rel
    # Strip leading ./
...
def bootstrap_notebook(
    ...
    if reload_modules:
        ...
        # Re-ensure path after reload (some imports may mutate sys.path)
