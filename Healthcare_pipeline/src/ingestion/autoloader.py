...
def _dir_has_files(path: str, spark: Optional[SparkSession] = None) -> bool:
    """True when the directory exists and already contains at least one data file."""
    try:
        p = Path(path)
        if p.exists() and any(f.is_file() and not f.name.startswith("_") for f in p.iterdir()):
            return True
    except Exception:
        pass
    try:
        ...
        if dbutils is not None:
            ...
            return any(not e.name.startswith("_") and e.size > 0 for e in entries)
    except Exception:
        pass
    return False
...
def stage_sample_files_to_landing(
    ...
    When the landing folder already contains files (e.g. freshly generated by
    the Faker notebook), staging is skipped unless overwrite=True, so manual
    data generation is never clobbered by the repo samples.
    """
    spark = spark or SparkSession.getActiveSession()
    ...
    for entity in ALL_ENTITIES:
        ...
        if not overwrite and _dir_has_files(dest_dir, spark):
            continue
