...
from src.utilities.table_registry import register_gold_tables
...
# MAGIC %md
# MAGIC ## 5. Register Gold as physical Unity Catalog tables

# COMMAND ----------

if cfg.unity_catalog.register_tables:
    try:
        # Only mart names (exclude any non-mart keys if present)
        ...
        try:
            display(spark.sql(f"SHOW TABLES IN `{catalog}`.gold"))  # noqa: F821
        except Exception:
