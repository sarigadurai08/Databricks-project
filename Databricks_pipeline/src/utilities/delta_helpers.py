...
def add_primary_key_constraint(
    ...
    try:
        spark.sql(
            f"ALTER TABLE {table_name} ADD CONSTRAINT pk_{table_name.replace('.', '_').replace('`', '')} "
            f"PRIMARY KEY ({cols})"
        )
    except Exception:
        pass
...
def enable_liquid_clustering(
    ...
    """Enable Liquid Clustering when available; skip gracefully otherwise."""
...
def maintain_entity(
    ...
    try:
        if optimize_enabled:
            zcols = OPTIMIZE_ZORDER_COLUMNS.get(entity) if zorder_enabled else None
