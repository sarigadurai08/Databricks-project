"""
Enterprise logging framework for the E-Commerce Lakehouse.

Provides structured application logging with optional persistence to Delta
tables for operational observability on Databricks.
"""
...
class EcommerceLogger:
    ...
    Usage:
        logger = EcommerceLogger(spark, pipeline_name="bronze_ingestion", run_id=run_id)
        logger.info("Starting ingestion", module="autoloader", details={"entity": "orders"})
    """

    def __init__(
        self,
        spark: Optional[SparkSession] = None,
        pipeline_name: str = "ecommerce_lakehouse",
        ...
        self._py_logger = logging.getLogger(f"ecommerce.{pipeline_name}")
    ...
    def flush(self) -> Optional[DataFrame]:
        """Persist buffered log records to Delta at PATHS.log_path()."""
...
def get_logger(
    ...
    return EcommerceLogger(spark=spark, pipeline_name=pipeline_name, run_id=run_id)


def ensure_log_table(spark: SparkSession) -> None:
    """Create empty Delta log table at PATHS.log_path() if it does not exist."""
    from src.utilities.delta_helpers import write_delta
