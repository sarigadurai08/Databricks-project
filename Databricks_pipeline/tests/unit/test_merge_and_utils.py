"""Unit tests for Delta MERGE helpers, soft_reset, and bronze metadata."""
...
from src.utilities.delta_helpers import merge_delta, soft_reset_delta_path, table_exists, write_delta


def test_add_bronze_metadata_with_hash(spark):
    df = spark.createDataFrame([("ORD1", "Ada")], ["order_id", "customer"])
...
def test_soft_reset_delta_path(spark, tmp_delta_dir):
    target = f"{tmp_delta_dir}/soft_reset"
    df = spark.createDataFrame([("1", "A"), ("2", "B")], ["id", "val"])
    write_delta(df, target, mode="overwrite")
    assert table_exists(spark, target)
    assert spark.read.format("delta").load(target).count() == 2

    status = soft_reset_delta_path(spark, target, schema_df=df)
    assert status in {"deleted_rows", "overwritten_empty", "overwrite_then_delete", "uncleared"}
    if status != "uncleared":
